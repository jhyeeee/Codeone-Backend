...
public class UserService {
	...
	// 회원가입 이메일 발송
    public void sendSignUpEmail(String email, String id, String name) throws Exception {
    	...
    	String mailContent = "<div style='text-align:center;'>" +
                ...
                "<a href='http://localhost/user/signUpemail?email="+email +"&id="+id+"&name="+name+" ' style='display:inline-block; background-color:#FF4136; color:#fff; font-size:16px; text-align:center; padding:12px 20px; border-radius:5px; text-decoration:none; margin-bottom:30px;'>계속하기</a>" +
    ...
    public void sendLoginEmail(String email, String emailKey) throws Exception {
    	...
    	String mailContent = "<div style='text-align:center;'>" +
                ...
                "<a href='http://localhost/codeone/user/regi?email=" + email + "&emailKey=" + emailKey + "' style='display:inline-block; background-color:#FF4136; color:#fff; font-size:16px; text-align:center; padding:12px 20px; border-radius:5px; text-decoration:none; margin-bottom:30px;'>계속하기</a>" +
                "<p style='font-size:14px; color:#999; margin-top:30px;'>본 이메일은 발신 전용입니다. 문의 사항은 고객센터를 이용해주세요.</p>" +
                "<p style='font-size:14px; color:#999;'>CodeOne | 서울특별시 강남구 테헤란로 1234</p>" +
                "</div>";
    	MimeMessage message = mailSender.createMimeMessage();
    ...
    public UserDto checkEmailKey(String emailKey) {
